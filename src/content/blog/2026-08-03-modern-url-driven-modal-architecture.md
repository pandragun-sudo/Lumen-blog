...
| 이전 채널명 고정 렌더링 | URL 파라미터가 DB 실측 데이터보다 높은 우선순위 점유 | SWR 실측값 최우선 렌더링으로 역전 + 라우팅 시 `params.delete()` 강제 집행 |
...
편의를 위해 컴포넌트 내부에서 URL 파라미터를 임의로 주입하던 레거시 코드를 모두 걷어내고, SWR을 통한 DB 실측값 직접 바인딩으로 데이터 흐름을 정돈했습니다.  
