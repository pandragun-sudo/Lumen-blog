...
| 제목 뒷단 해시태그 |  | 유튜브 검색 알고리즘에 영상의 정확한 주제를 전달 |
| 추가 해시태그 |  | 개봉 시기에 맞춘 시의성 트래픽 확보 |

제목 앞부분은 사람이 읽고 반응하는 '심리적 후킹'에 집중시키고, 검색 엔진이 읽어야 할 고유명사는 뒷부분의 '해시태그'로 완전히 분리해 넘겨주는 방식입니다.

## 2,300만 뷰 대박 쇼츠인데 좋아요가 0개로 잡힌 이유

타 채널의 대형 쇼츠들을 모니터링할 때도 이와 비슷한 원리가 발견됩니다. 최근 루멘 인사이트 인앱 플레이어로 레퍼런스 영상(*"당황한 진상'VIP'ㅋㅋ"*, 라마의드라마)을 분석하던 중 독특한 화면을 캡처했습니다.

![루멘 인사이트 인앱 플레이어에서 분석된 2,362만 뷰 쇼츠의 지표 분석 화면](../../assets/blog/SCR-20260825-kspn.png)

무려 **2,362.2만 뷰**에 달하는 초대형 영상인데, 시스템에서 수집된 **좋아요(LIKES) 숫자는 0**으로 찍혀 있습니다. 

이는 크리에이터가 좋아요 표시를 비공개로 설정하여 유튜브 API에서 값이 비어 온 것이지만, 쇼츠를 만드는 사람 입장에서 곱씹어볼 만한 본질을 담고 있습니다:
...
4. **잘 만든 레퍼런스 뜯어보기**: 루멘 인사이트 같은 도구로 수천만 뷰를 기록한 영상들의 제목 문장 구조와 실제 유입 검색어의 상관관계를 주기적으로 관찰하고 내 채널에 맞춰 변형해 봅니다.

---

루멘 인사이트(Lumen Insights)에서는 실시간 급상승 쇼츠의 메타데이터와 검색 키워드 흐름을 한눈에 추적할 수 있습니다. 피드와 검색을 모두 만족시키는 데이터 기반 쇼츠 제목을 직접 설계해 보세요.
